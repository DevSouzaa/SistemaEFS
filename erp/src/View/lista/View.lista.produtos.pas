  ...
  try
    ...
    try
      ...
      begin
        ...
        // Não limpe a lista `Produtos`, pois isso tentaria liberar os objetos que já estão em `FProdutos`

        ViewFrameDuasColuna.ControlList1.ItemCount := FProdutos.Count;
        CadPanel_Dados.ActiveCard := Card_DuasColuna;
      end;
    finally
      // Não precisa liberar `Produtos`, porque `FProdutos` já contém os objetos.
      // Se `Produtos` for liberada, os objetos dentro dela também serão liberados, causando problemas.
      Produtos.OwnsObjects := False; // Desativa a propriedade de liberação automática antes de liberar `Produtos`
      Produtos.Free;
    end;
  finally
